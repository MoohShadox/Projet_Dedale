...
public class Planificateur_Simple {
    ...
    public Planificateur_Simple(Simple_Cognitif_Agent myAgent){
        this.myAgent = myAgent;
        kb = myAgent.Kb;
    }

    public void initialiserValeur(Integer objectif, Integer ... contraintes){
        this.penality.clear();

        if(values == null)
            values = new HashMap<>();

        for (Integer s:successeurs.keySet())
        {
            values.put(s,0.0);
            for(Integer noeud : successeurs.get(s))
                values.put(noeud,0.0);
        }

        values.put(objectif,10.0);
        for (Integer i : contraintes)
        {
            penality.add(i);
            values.put(i,-5.0);
        }
        //System.out.println("Valeurs aprés intiialisation "+values);
    ...
    public void updateSuccesseurs(){
        ...
        if(myAgent.successeurs_recus != null)
            successeurs.putAll(myAgent.successeurs_recus);
        System.out.println("sachant les successeurs : "+successeurs);
    ...
    public void getNextStep(Integer objectif , Integer ... contraintes){
        this.plans.clear();
        this.updateSuccesseurs();
        this.initialiserValeur(objectif,contraintes);
        this.calculerPlans();
    ...
    {
        System.out.println(myAgent.getLocalName()+">"+"Values au début de la génération du plan de suivi depuis : "+values+" la source "+source);
