...
public class PolicyEvaluationHunting {
    ...
    public void initialiserValeur(){
        ...
        System.out.println("odeurs : "+myAgent.odeurs);
        System.out.println("positions : "+myAgent.positions_agents);
        ...
        for(String s:myAgent.odeurs.keySet()){
            {
                for (Integer odeur:myAgent.odeurs.get(s))
                {
                    this.values.put(odeur,20.0);
                }
            }
        }

        for (String s:myAgent.positions_agents.keySet()){
            if(!s.equalsIgnoreCase(myAgent.getLocalName()))
            {
                penality.add(myAgent.positions_agents.get(s));
                values.put(myAgent.positions_agents.get(s),-5.0);
            }
        }

        values.put(Integer.parseInt(myAgent.getCurrentPosition()),-1.0);
